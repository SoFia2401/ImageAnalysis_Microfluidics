...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 27 12:20:25 2023
...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 20 18:50:18 2023

@author: sofiahernandezgelado
"""

#Circle detection with SAM enhanced for circles
import csv
import numpy as np
import os
import cv2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--folder-path', type=str, required=True)  # Directory containing images
    ...
    sam = sam_model_registry["vit_h"](checkpoint="/home/sofiahernandezgelado/Documents/SAM/sam_vit_h_4b8939.pth")
    ...
    csv_file_images = os.path.join(args.folder_path, "SAM_segmentation_metrics.csv")
    ...
    csv_file_overall = os.path.join(args.folder_path, "SAM_segmentation_metrics_overall.csv")
    ...
    for filename in os.listdir(args.folder_path):
        if filename.endswith(".tif") and not filename.endswith(pattern.format(args.scale)) and not filename.endswith("srgan.tif"):
            # High-resolution image path
            high_res_image_path = os.path.join(args.folder_path, filename)

            # Get the corresponding bicubic and model image paths
            bicubic_image_path = os.path.join(args.folder_path, filename.replace('.tif', f'_bicubic_x{args.scale}.tif'))
            model_image_path = os.path.join(args.folder_path, filename.replace('.tif', f'_{args.model}_x{args.scale}.tif'))
           ...
           # Calculate the difference in radii for matched circles
            ...
            masked_high_res_path = os.path.join(args.folder_path, f"masked_{filename}")
            masked_bicubic_path = os.path.join(args.folder_path, f"masked_{filename.replace('.tif', f'_bicubic_x{args.scale}.tif')}")
            masked_model_path = os.path.join(args.folder_path, f"masked_{filename.replace('.tif', f'_{args.model}_x{args.scale}.tif')}")
