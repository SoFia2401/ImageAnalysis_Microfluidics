...
from .utils.metrics import AverageMeter, calc_psnr, calc_ssim
...
class Trainer:
    ...
    def __init__(
        ...
        self.best_epoch_psnr = 0
        self.best_epoch_ssim = 0
        self.best_psnr = 0.0
        self.best_ssim = 0
            ...
            for epoch in range(epochs_trained, num_train_epochs):
                ...
                with tqdm(total=(len(train_dataset) - len(train_dataset) % train_batch_size)) as t:
                    ...
                    for data in train_dataloader:
                        ...
                        t.set_postfix(loss=f'{train_loss.avg:.6f}')
                        t.update(len(inputs))
                ...
                self.eval(epoch, val_psnr, val_loss, val_ssim)

    def eval(self, epoch, val_psnr, val_loss, val_ssim):
        ...
        print(f'scale:{str(scale)}      eval psnr: {val_psnr.per_epoch[epoch]:.2f}     ssim: {val_ssim.per_epoch[epoch]:.4f}')

        if val_psnr.per_epoch[epoch] > self.best_psnr:
            self.best_epoch_psnr = epoch
            self.best_psnr = val_psnr.per_epoch[epoch]

            print(f'best epoch: {self.best_epoch_psnr}, psnr: {val_psnr.per_epoch[epoch]:.6f}, ssim: {val_ssim.per_epoch[epoch]:.6f}')
            self.save_model()
    ...
    def get_eval_dataloader(self) -> DataLoader:
        ...
        if eval_dataset is None:
            print("Problem using train dataset")
